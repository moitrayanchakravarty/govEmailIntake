 ...
 * @access  Private (office_admin & portal_manager — enforced by
 *          requireAuth + requireRole in registryRoutes.js)
 */
exports.searchRegistry = async (req, res) => {
    try {
        ...
        // req.user is populated by requireAuth (authMiddleware.js), and its
        // .role/.officeName values come straight from Better Auth's admin
        // plugin + our additionalFields config — matching the exact role
        // strings defined in config/permissions.js ('office_admin', not 'Office_Admin').
        if (req.user.role === 'office_admin') {
            dbQuery.officeName = req.user.officeName; // Can ONLY see their own office's entries
        }
        // portal_manager: no restriction applied — sees every office's records.

        // 3. Add optional dynamic filters (REG-02)
        if (req.query.status) {
            dbQuery.status = req.query.status;
        }
        if (req.query.designation) {
            // Case-insensitive partial match (e.g. "Director" catches "Director of IT")
...
exports.updateRegistryOnApproval = async (emailAddress, updateData) => {
    try {
        return await MasterRegistry.findOneAndUpdate(
            ...
            { $set: updateData },
            { new: true, upsert: true } // Upsert creates a new entry if it doesn't exist yet
        );
    } catch (error) {
